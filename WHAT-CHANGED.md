...
- **Tenet set re-cut (still six).** Old Tenet 1 (accessible) + old Tenet 4 (assumes nothing technical) merged into the new **Tenet 1 — "Meet a non-technical reader where they are."** New **Tenet 4 — "Coherent through-line"** added. **T5 (steer-not-code) and T6 (AI-limitations) keep their numbers and meaning** — so every existing "Tenet 5/6" reference stays valid. Renumber map: old T1+T4 → new T1; new T4 = coherence; T2/T3/T5/T6 stable. Log entries dated before today use the old numbering.
- **Narrative coherence is now a first-class tenet** with teeth: a new `docs/COURSE-AUTHORING.md` **Part 3 — "Narrative coherence: the module spine"**, a `## What this module builds` arc added to every module README (M0/M1/M2/M3/M3.5), and coherence notes in both lesson templates. GUIDANCE — human review against the spine; no lint.
- **The anchor-lesson exception** is named as a principle (COURSE-AUTHORING Part 7; CLAUDE.md hard rule 14; `docs/audience-vocabulary.md`). It resolves the contradiction where `hallucination` is Forbidden in the M2 tier yet introduced by its anchor lesson M2 L6.
- **voice-lint check #6 (jargon-density) extended from M0/M1 to M0–M3.5.** The vocab contract was previously unenforced — not even as WARN — from Module 2 onward. The check is WARN-only, so the gate still exits 0; **the WARN baseline rose from ~50 to 149** (M2/M3/M3.5 prose predates the check covering it). The new WARNs are expected editorial backlog, non-blocking.
- **De-dup + staleness sweep.** TENETS trimmed ~40% to Plain statement + Why + a one-line Operationalized/Enforced pointer per tenet (the duplicative how/where-enforced blocks moved to their canonical homes). COURSE-AUTHORING Parts renumbered (new Part 3 pushes old Parts 3–12 → 4–13); every `Part N` pointer in CLAUDE.md, both lesson templates, `docs/audience-vocabulary.md`, and the `course-lesson-authoring` skill updated to match. Part 9 (voice-lint) corrected from "seven checks" to nine (rows added for #8 m3-dual-agent and #9 m35-diagnostic-framing). The double-`RLS` typo in the audience-floor list fixed by making `docs/audience-vocabulary.md` the single canonical term list (TENETS + COURSE-AUTHORING now keep examples and point to it).

**Affected lessons / artifacts:** `docs/TENETS.md`, `CLAUDE.md`, `docs/COURSE-AUTHORING.md`, `docs/audience-vocabulary.md`, `lesson-template.md`, `lesson-template-m0.md`, all five `modules/*/README.md`, `scripts/voice-lint.sh`, `CONTRIBUTING.md`, `.claude/skills/course-lesson-authoring/`. No lesson *bodies* changed.

**What learners should do:** Nothing — these are author-facing contract docs. Each module README now opens with a "What this module builds" arc that doubles as orientation.
...
**Why now:** Phase 02.3 locked the framework — `docs/TENETS.md` + CLAUDE.md Hard Rule 13 (Execution-Responsibility Boundary M4+) + Hard Rule 14 (AI-Limitation Pedagogy) + `docs/COURSE-AUTHORING.md` Parts 5 / 6 / 7 + `docs/audience-vocabulary.md` extended through M4–M7. Phase 02.4 is the corresponding polish phase: brings the shipped lesson bodies into compliance with the contract before Phase 3 build-phase work begins. The audit at `.planning/soul-of-course-audit-2026-05-18.md` named the per-lesson specific drift evidence; each polish in this phase closes one of the audit's 13 findings. The audit's three structural problems — "soul in pieces" (Problem 1), "Hard Rule 12 stops at M3.5" (Problem 2 — that was Phase 02.3's territory), "Tenet 6 had no anchor" (Problem 3) — all close with this phase: Problem 1 closed by Phase 02.3's TENETS.md; Problem 3 closed by Phase 02.4's M2 L6 anchor + M3 L3 hallucination-mechanism un-punt.

**Affected lessons / artifacts:**
- `modules/01-mental-models/01-how-the-web-works.md` (M1 L1, SC #1 — tenet 1 jargon-strip + tenet 5 floor; commit `a89ad5d`)
- `modules/01-mental-models/02-where-data-lives.md` (M1 L2, SC #2 — tenet 1 double-definition cut + tenet 5 mechanism soften; commit `a936278`)
- `modules/01-mental-models/03-who-can-do-what.md` (M1 L3, SC #3 — tenet 1 plain-language safeguards; commit `5e5e573`)
- `modules/01-mental-models/04-how-it-goes-live.md` (M1 L4, SC #4 — tenet 1 jargon-strip + tenet 5 failure-mode flatten; commit `3c5b3cc`)
- `modules/02-toolchain/03-runtime-node.md` (M2 L3, SC #5 — tenet 5 TypeScript SYMPTOM-only; commit `dc881b8`)
- `modules/02-toolchain/06-ai-coding-agents.md` (M2 L6, SC #6 — **Tenet 6 anchor + Hard Rule 14 forward-refs**; commit `59c2339`)
- `modules/03-the-loop/01-introducing-the-loop.md` (M3 L1, SC #7 — tenet 6 steer skill shown; commit `1a3c7b5`)
- `modules/03-the-loop/02-planning-vs-execution.md` (M3 L2, SC #8 — tenet 5 working-memory-first reorder; commit `3000313`)
- `modules/03-the-loop/03-reading-plans-recognizing-wrong.md` (M3 L3, SC #9 — **tenet 6 hallucination un-punt**; commit `5698b47`)
- `modules/03-the-loop/04-steering-and-recovery.md` (M3 L4, SC #10 — tenet 6 context-commitment + console-pointer; commit `f57a17c`)
- `modules/03.5-reading-code/01-reading-a-file-tree.md` (M3.5 L1, SC #11 — tenet 5 final-seam tightening; commit `d3bcb94`)
- `modules/03.5-reading-code/03-error-message-to-file-pointer.md` (M3.5 L3, SC #12 — tenet 5 example-text mechanism trim; commit `325117d`)
- `modules/03.5-reading-code/04-use-client-and-server-split.md` (M3.5 L4, SC #13 — tenet 4 audience-floor + pre-exercise guard; commit `99d3d5b`)
- `GLOSSARY.md` (added `### drift` + `### risk-blindness` anchors in the M2 L6 same-PR commit per D-36 convention; `### hallucination` already existed from Phase 2 close and is unchanged)

**Affected analogies (carryforward, unchanged):** Every locked analogy from D-07 (M1 bundles 1–4) and D-40..D-48 (M2 + M3.5) survives verbatim across all 12 polished lessons. Restaurant + customer + waiter + kitchen + dish (M1 L1) / filing cabinet + drawer + cards + receptionist (M1 L2) / door staff + VIP list + hand stamp (M1 L3) / private kitchen + recipe binder + prep cooks + public restaurant (M1 L4) / sheet music + musician (M2 L3) / junior teammate (M2 L6) / office directory (M3.5 L1) / receipt with line item circled (M3.5 L3) / framed picture vs. touchscreen (M3.5 L4). The M2 L3 TypeScript reframe and M2 L6 Tenet 6 anchor EXTEND their lessons' analogies (musician + extra annotations on the music; teammate's weak spots) rather than replacing them. M3 series has no D-07 / D-40..D-48 analogy (loop-pedagogy lessons; the four-step loop IS the structure).

**Voice-lint status:** Exit 0 repo-wide. Check #9 (M3.5 diagnostic-framing) emits zero WARNs against M3.5 L1 / L3 / L4 (the Phase 02.2 zero-WARN baseline preserved). ~50 WARN editorial-backlog lines persist (D-39 / D-64 carryover from Phase 02.1 — module-README and cross-cutting-doc callout-missing on already-defined Safe terms; non-blocking).

**What contributors should do:** Read `docs/TENETS.md` (six tenets) FIRST when authoring or revising any lesson. Then `CLAUDE.md` Hard Rules 13 + 14 (Execution-Floor Boundary + AI-Limitation Pedagogy). Then `docs/COURSE-AUTHORING.md` Parts 5 / 6 / 7 (Execution-Floor / AI-Limit Pedagogy / What NOT to Teach). Then `docs/audience-vocabulary.md` M4–M7 sections. Future lesson edits must satisfy the contract these documents lock. When naming an agent failure mode (hallucination, drift, context-window overflow, training cutoff, confident-wrong, risk-blindness, prompt-injection): give the learner a concrete smell-test in the same lesson OR a Hard Rule 14 forward-reference (the locked opener `> **Heads up — you'll meet this again.**` + bold-name the limit + one-sentence symptom in the analogy frame + named smell-test location). When polishing a lesson that ships with a locked analogy: preserve the analogy verbatim. If a polish would require changing an analogy's mapping, escalate before editing.
