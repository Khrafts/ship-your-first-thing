...
An open-source course teaching a non-technical learner to ship a deployed app using AI coding agents. The course is **plain markdown**, no static-site-generator dependency, browsable on github.com. A Next.js + Postgres + Drizzle + Auth.js platform in `site/` (built; deploys to Railway, will serve at `shipyourfirstthing.com`) renders the same markdown — but every lesson must work on github.com first.
...
1. **`docs/TENETS.md`** — the soul of the course. Six tenets that everything else serves. Read this BEFORE this file. If a rule in CLAUDE.md conflicts with a tenet, the tenet wins.
2. **This file** — agent rules + pointers.
3. **`.planning/PROJECT.md`** — core value, locked decisions, requirements evolution rules. Source of truth for what the course is for and what it must NOT do.
4. **`.planning/REQUIREMENTS.md`** — locked requirements (LESSON-01…, DOC-01…, OPS-01…, etc.).
5. **`.planning/ROADMAP.md`** — milestone + phase structure, success criteria, dependency chains.
...
12. **Agent-Responsibility Boundary (M3.5 floor).** Every lesson section is either "the agent handles this" or "the learner does this" — never both. The agent owns: reading errors, parsing code, diagnosing causes, framework mechanics, choosing which file to edit, build internals. The learner owns: stating intent, checking the agent edited the right file, recognizing wrongness, asking for help. Mechanics the agent owns must not be taught at the audience floor. Where a symptom term must be named to ground a steer (for example, `'use client'` or `hydration`), introduce it as a SYMPTOM, never as a concept to understand from first principles. The full boundary spec lives in `.planning/PROJECT.md` Key Decisions; the per-topic M3.5 floor lives in `docs/COURSE-AUTHORING.md` Part 5. `scripts/voice-lint.sh` check #9 emits WARN-level signals when M3.5 lessons drift into agent-territory framing.
13. **Execution-Responsibility Boundary (M4+ build phases).** When a learner builds via the agent (Modules 4, 5, 6), every lesson distinguishes Agent-Authoring territory from Learner-Steering territory. The **agent owns**: schema authoring, RLS policy syntax, async/await mechanics, hook internals (`useState`, `useEffect`, `useOptimistic`, etc.), Server-Action plumbing, type narrowing, dependency resolution, build internals, deployment plumbing. The **learner owns**: stating intent at the feature level ("logged-out visitors should see public posts but not the like button"), observing the running app matches intent (open it, click around, sign out, sign in as a second user), applying the phase's **smell-test inventory** (named observable patterns the learner scans for; absence = ask the agent why), committing each working chunk to git, knowing when to `/clear` and start over. Mechanics the agent owns are not taught at the audience floor — not in lesson body, not in exercises, not in worked examples. Where a build-phase term must be named to ground a steer (for example, `WITH CHECK`, `useOptimistic`, `revalidatePath`, `cookies()`), introduce it as a SYMPTOM (a label you scan for in the agent's diff), never as a concept to understand from first principles. The smell-test inventory for each build phase is locked in `.planning/phases/NN-name/NN-CONTEXT.md` BEFORE the planner runs — no execution-floor lesson ships without the inventory being locked. Full guidance: `docs/COURSE-AUTHORING.md` Part 6 (Execution-Floor Boundary).
14. **AI-Limitation Pedagogy.** Whenever a lesson names an agent failure mode (hallucination, drift, context-window overflow, training cutoff, confident-wrong, risk-blindness, prompt-injection), it must EITHER (a) give the learner a concrete smell-test for that failure mode within the same lesson, OR (b) explicitly defer with a forward-reference to the lesson where the smell-test lives. **Never name a failure mode without arming the learner for it.** The smell-test is the bridge between abstract knowledge ("the agent might hallucinate") and practical skill ("the agent's `apiKey` reference doesn't match anything in our `.env` — let me ask where that name came from"). The six-limitation taxonomy and per-limit smell-test patterns live in `docs/COURSE-AUTHORING.md` Part 7 (AI-Limitation Pedagogy). M2 L6 anchors three of them (hallucination / drift / risk-blindness); M3 L3 teaches the hallucination smell-test in depth; M5 watch-it-fail walkthroughs (LESSON-13) ship three more. When you mention a failure mode in passing without a smell-test in the same lesson, the forward-reference is mandatory — not a suggestion. **Anchor-lesson exception:** when a failure-mode term is Forbidden in a module's vocabulary tier, only that module's *designated anchor lesson* introduces it (with a callout, in the "notice the name" framing); every other lesson in the module defers in plain words — see `docs/COURSE-AUTHORING.md` Part 7 § The anchor-lesson exception.

## Before you commit

Run `./scripts/voice-lint.sh`. Exit code 0 is required. A backlog of `WARN (jargon-density …)` lines is expected and does not block the gate — they mark where prose hasn't yet converged on the strict vocab contract (check #6 now surfaces M0–M3.5, so the backlog is larger than the old M0/M1-only ~50; for the live count run `./scripts/voice-lint.sh | grep -c '^WARN'`). **Violations** (lines starting `VIOLATION (...)`) do block. Check #10 (whatchanged-entry-shape) blocks non-conforming `WHAT-CHANGED.md` entries — the thin-entry contract lives in `CONTRIBUTING.md` § Adding a WHAT-CHANGED entry. (Separately, the `whatchanged-reminder` workflow — `scripts/whatchanged-reminder.sh`, PR-only — nudges, never blocks, when a PR changes learner-facing content but adds no entry; it complements #10, which validates an entry that exists.) Don't read exit 0 as "vocab contract satisfied" — #6 is WARN-only, and M4+ is human-review until those modules exist.
