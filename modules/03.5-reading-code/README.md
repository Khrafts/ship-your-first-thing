...
- **Lesson 4 — `'use client'` and the server/client split:** recognize whether a file needs `'use client'`, name the symptom of a missing one (an interactive element that silently does nothing, or a hydration error), and steer the agent to the fix → sets up Phase 3, where you use this observation floor on the thread project every day.
