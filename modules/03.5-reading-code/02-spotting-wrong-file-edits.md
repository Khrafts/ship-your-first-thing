...
Module 3.5 Lesson 1 named what folders look like. This lesson tightens that skill to the FILE level. When you ask an agent to make a change, the agent reports which files it touched. One of the most common ways an AI coding agent goes wrong — beyond inventing details — is editing the wrong file. This lesson teaches you to catch that in seconds, by comparing what the agent SAID it was doing against where it actually changed bytes.

## Core read

When an AI coding agent edits files, it shows you two things you can scan at a glance. First, the **file panel** (a one-line definition: the editor's source-control or file-tree panel showing which files have just changed, often with an `M` marker or color highlight, [→ GLOSSARY](../../GLOSSARY.md#file-panel)) — a visual list of every file that was touched in the last edit pass. Second, the **diff summary** (a one-line definition: a one-line per-file report from the agent showing the filename and the number of lines added or removed, [→ GLOSSARY](../../GLOSSARY.md#diff-summary)) — what the agent itself prints in the conversation after it finishes.
